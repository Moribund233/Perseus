...
async def create_repository(repo_data: dict, db: Session):
    ...
    try:
        ...
        print(f"Warning: Failed to create physical git repository at {physical_path}: {e}")
    except Exception as e:
        # 其他错误，记录但不阻止
        print(f"Warning: Unexpected error creating git repository: {e}")
...
async def delete_repository(repo_id: int, db: Session):
    ...
    try:
        ...
        if os.path.exists(physical_path):
            ...
            print(f"物理仓库已删除: {physical_path}")
    except Exception as e:
        # 物理仓库删除失败，记录错误但不阻止数据库删除
        print(f"Warning: Failed to delete physical repository at {physical_path}: {e}")
