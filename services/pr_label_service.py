...
async def create_label(repo_id: int, data: dict, db: AsyncSession) -> dict:
    """创建 PR 标签"""
...
async def update_label(repo_id: int, label_id: int, data: dict, db: AsyncSession) -> dict:
    """更新 PR 标签"""
...
async def add_label_to_pr(pr_id: int, label_id: int, db: AsyncSession) -> dict:
    """为 PR 添加标签"""
    result = await db.execute(
        select(PullRequest)
...
async def remove_label_from_pr(pr_id: int, label_id: int, db: AsyncSession) -> dict:
    """从 PR 移除标签"""
    result = await db.execute(
        select(PullRequest)
        .filter(PullRequest.id == pr_id)
...
async def get_prs_by_label(label_id: int, db: AsyncSession) -> list[dict]:
    """获取使用指定标签的所有 PR"""
    result = await db.execute(
        select(PRLabel)
