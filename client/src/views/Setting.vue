...
// 从 store 获取 CORS 配置和代理状态
const corsConfigFromStore = computed(() => serviceStore.corsConfig)
 ...
 * 同步 CORS 配置
 */
const syncCORSConfigFromStore = (): void => {
  if (corsConfigFromStore.value) {
    corsConfig.value = { ...corsConfig.value, ...corsConfigFromStore.value }
...
const saveCORSConfig = async (): Promise<void> => {
  ...
  try {
    ...
    if (result.success) {
      successMessage.value = 'CORS 配置保存成功（需要重启服务才能生效）'
    ...
    error.value = '保存 CORS 配置失败: ' + String(err)
  } finally {
    isSaving.value = false
  }
}
...
<template>
  <div class="settings">
    ...
    <!-- CORS 配置 -->
    <div v-show="activeTab === 'cors'" class="config-section">
      <!-- 代理模式提示 -->
