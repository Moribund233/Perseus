# LanGit API v1 开发规划路线图

> 本文档记录 LanGit 项目 API v1 版本（基础功能）的完整开发历程、技术实现和当前状态
> 
> 最后更新：2026-02-10（API v1 功能已完整，WebSocket 基础集成完成）

---

## 📋 项目概述

API v1 版本专注于构建一个功能完整的局域网 Git 服务器，包含核心 Git 操作、Web 代码浏览和团队协作功能。
...
| API v2 | 🟡 规划中 | 扩展 WebSocket 实时功能（详见 [API v2 开发规划](../api_v2/ROADMAP.md)）|
...
| 问题 | 修复内容 | 文件 |
|-----|---------|------|
| 路由前缀重复 | 移除 `api_v1_router` 的 `/api` 前缀，避免双重前缀 | `api/api_v1.py` |

**修复说明**：
- 原问题：`api_v1_router = APIRouter(prefix="/api")` + 控制器 `prefix="/api/xxx"` = `/api/api/xxx`
- 修复后：`api_v1_router = APIRouter()` + 控制器 `prefix="/api/xxx"` = `/api/xxx`
...
**连接端点**：
```
WS /ws/v1/notifications              # 通知频道
WS /ws/v1/sync/{repo_id}             # 同步状态频道
```
